...
## Known exemptions (technical debt)

The EXCLUDE array in `scripts/check_credentials.sh` allowlists files that
...
- ~~`API_REFERENCE.md`~~ — **CLEARED 2026-05-10** by Phase-3 plan 03-03.
  The top-level `API_REFERENCE.md` is now regenerated by
  `scripts/regenerate_api_reference.py` from `/openapi.json`; example curl
  bodies use `${TEST_ADMIN_EMAIL}` / `${TEST_ADMIN_PASSWORD}` placeholders
  sourced from `memory/test_credentials.md`. Exemption removed from the
  scanner EXCLUDE list. The file now passes `bash scripts/check_credentials.sh`
  on its own merits.
- `frontend/public/docs/API_REFERENCE.md` — example curl bodies inline the
  admin password (this is the in-app doc viewer's frozen copy of the old
...
The `LOGIN_BUG.md` and `CREDENTIAL_HYGIENE.md` self-references discuss the
patterns verbatim as documentation; they are exempt by design (not debt).
The scanner script itself is exempt to avoid self-matching.
