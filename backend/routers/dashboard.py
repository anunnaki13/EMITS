...
def _dashboard_period_bounds(period: Optional[str]):
...
def _period_match(field: str, period: Optional[str]) -> dict:
    bounds = _dashboard_period_bounds(period)
    if not bounds:
        return {}
    start, end = bounds
...
def _aging_days(value: Optional[str]) -> Optional[int]:
    if not value:
        return None
    try:
        return max((date.today() - date.fromisoformat(value[:10])).days, 0)
    except ValueError:
        return None
        ...
        [
            {
                ...
                "aging_days": _aging_days(item.get("completed_unloading")),
            }
            for item in all_coa
