"""Factory for trucking documents (Phase 4 D-02)."""
import os
import uuid
...
def make_trucking(**overrides) -> dict:
    """Insert one minimal trucking document into the test DB. Returns doc dict (without _id)."""
    doc = {
        "id": str(uuid.uuid4()),
        "periode_ta": "Jan-2026",
    ...
    try:
        mongo_client[_get_test_db_name()].trucking.insert_one(doc)
    finally:
        mongo_client.close()
    doc.pop("_id", None)
    return doc
