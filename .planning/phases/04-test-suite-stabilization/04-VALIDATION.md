...
| 04-04-01 | 04 | 2 | TEST-06 | T-llm-budget-leak-01 / mitigated | `/api/ai/query` and `/api/smart-blending/recommend` return canned shapes via FakeAIClient (AI_FAKE=1); ZERO outbound LLM calls observed in run | integration | `AI_FAKE=1 pytest tests/test_ai_endpoints.py -q && grep -c 'emergent.*api' tests/.run.log == 0` | ❌ W0 | ⬜ pending |
