...
import { Anchor, Plus, Search, Upload, MoreHorizontal, Edit, Trash2, Loader2, FileSpreadsheet, AlertTriangle, Eye, RotateCcw } from "lucide-react";
    ...
    <div className="space-y-6" data-testid="barge-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[minmax(220px,1fr)_180px_160px_160px_auto] gap-3 items-end">
        <div className="relative">
          ...
          <Input placeholder="Shipment, barge, supplier" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-slate-950/50 border-slate-800 text-white placeholder:text-slate-600" data-testid="barge-search-input" />
        </div>
        <div>
          <Label className="text-slate-400 text-xs">Supplier</Label>
          <Input placeholder="Nama supplier" value={supplier} onChange={(e) => setSupplier(e.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder:text-slate-600" data-testid="barge-supplier-filter" />
        </div>
        <div>
          <Label className="text-slate-400 text-xs">Dari</Label>
          <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="bg-slate-950/50 border-slate-800 text-white" data-testid="barge-date-from-filter" />
        </div>
        <div>
          <Label className="text-slate-400 text-xs">Sampai</Label>
          <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="bg-slate-950/50 border-slate-800 text-white" data-testid="barge-date-to-filter" />
        </div>
        <Button type="button" variant="outline" onClick={resetFilters} className="border-slate-700 text-slate-300 hover:bg-slate-800" data-testid="barge-reset-filters">
