  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSend = {
        ...
        lama_terbit_row: parseFloat2(formData.lama_terbit_row),
        ...
        durasi_pembongkaran_hari_2: parseFloat2(formData.durasi_pembongkaran_hari_2),
        waktu_tunggu_jam: parseFloat2(formData.waktu_tunggu_jam),
        durasi_terbit_coa: parseFloat2(formData.durasi_terbit_coa)
      };
              ...
              <DialogContent className="bg-[#0B1221] border-white/10 max-w-md">
                ...
                <div className="space-y-4 pt-4">
                  <div className="border-2 border-dashed border-slate-700 rounded-xl p-8 text-center hover:border-green-500/50 transition-colors">
                    ...
                    <p className="text-slate-500 text-xs mb-4">Header: Periode, Shipment Code, Voyage Code, Lot, Suppliers, Shipper, TB, BG, Biomass, dll.</p>
                    ...
                    <Tabs defaultValue="shipment" className="w-full">
                      ...
                      <TabsContent value="shipment" className="space-y-4 mt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <FormField label="Periode" name="periode" placeholder="2025-01-01" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <FormField label="Lot.1" name="lot_1" />
                          <FormField label="TB (Tug Boat)" name="tb" />
                          <FormField label="BG (Barge)" name="bg" />
                          <FormField label="Biomass" name="biomass_type" placeholder="WOODCHIP, SAWDUST" />
                        </div>
                      </TabsContent>
                      ...
                      <TabsContent value="kualitas" className="space-y-4 mt-4">
                        ...
                        <h4 className="text-sm font-mono text-green-400 mt-4">TM & IM (%wt)</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <TabsContent value="dokumen" className="space-y-4 mt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <FormField label="Lama Terbit ROW" name="lama_terbit_row" type="number" />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <FormField label="Durasi Terbit COA" name="durasi_terbit_coa" type="number" />
                        </div>
                      </TabsContent>
                    </Tabs>
              ...
              {loading ? (
                ...
                paginatedBiomassa.map((biomassa) => (
                  <TableRow key={biomassa.id} className="border-white/5 hover:bg-slate-900/50" data-testid={`biomassa-row-${biomassa.id}`}>
                    ...
                    <TableCell className="text-white font-medium text-sm max-w-[150px] truncate">{biomassa.shipment_code}</TableCell>
                    <TableCell className="text-slate-300 text-sm max-w-[150px] truncate">{biomassa.suppliers}</TableCell>
                    <TableCell>
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400">
                        {biomassa.biomass_type || "-"}
                      </span>
                    </TableCell>
                    <TableCell className="text-slate-300 text-sm font-mono">{biomassa.jembatan_timbang_mt?.toLocaleString() || '-'}</TableCell>
            ...
            {viewingBiomassa && (
              <div className="space-y-6 pt-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-900/50 rounded-lg">
                  ...
                  <div><p className="text-slate-500 text-xs">Durasi Pembongkaran (Hari)</p><p className="text-white text-sm">{viewingBiomassa.durasi_pembongkaran_hari?.toFixed(2) || "-"}</p></div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 bg-slate-900/50 rounded-lg">
                  ...
                  <div><p className="text-slate-500 text-xs">Lama Terbit ROW</p><p className="text-white text-sm">{viewingBiomassa.lama_terbit_row?.toFixed(2) || "-"}</p></div>
                  ...
                  <div><p className="text-slate-500 text-xs">Durasi Terbit COA</p><p className="text-white text-sm">{viewingBiomassa.durasi_terbit_coa?.toFixed(2) || "-"}</p></div>
                </div>
              </div>
            )}
          </ScrollArea>
